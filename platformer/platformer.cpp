...
void tiles_in_rect(Rect vp, std::function<void(Point)> f) {
  int minx = vp.x / 8;
  int maxx = ((vp.x + vp.w) / 8);
  int miny = vp.y / 8;
  int maxy = ((vp.y + vp.h) / 8);

  auto map_bounds = map->get_bounds();

  minx = minx < 0 ? 0 : minx;
  miny = miny < 0 ? 0 : miny;

  maxx = maxx > map_bounds.w - 1 ? map_bounds.w - 1 : maxx;
  maxy = maxy > map_bounds.h - 1 ? map_bounds.h - 1 : maxy;
...
struct Player {
  ...
  */
  Point camera() {
    auto map_bounds = map->get_bounds();
    static Rect b(screen_size.w / 2, screen_size.h / 2, map_bounds.w * 8 - screen.bounds.w, map_bounds.h * 8 - screen.bounds.h);
  ...
  void update() {
    ...
    tiles_in_rect(bounds_lr, [this, bounds_lr](Point tile_pt) -> void {
    ...
    tiles_in_rect(bounds_ud, [this, bounds_ud](Point tile_pt) -> void {
      if (map->has_flag(tile_pt, TileFlags::SOLID)) {
        Rect rb(tile_pt.x * 8, tile_pt.y * 8, 8, 8);
...
void render(uint32_t time) {
  ...
  // render lights
  auto effects_layer = map->get_layer("effects");
  for (uint8_t y = 0; y < 24; y++) {
    for (uint8_t x = 0; x < 48; x++) {
      uint32_t ti = effects_layer->tile_at(Point(x, y));
  ...
  draw_layer(map->get_layer("background"));
  draw_layer(map->get_layer("environment"));
  draw_layer(effects_layer);
  draw_layer(map->get_layer("objects"));
...
std::vector<std::pair<Vec2, Vec2>> get_occluders(Point pt, float radius) {
  ...
  tiles_in_rect(light_bounds, [&occluders, &pt](Point tile_pt) -> void {
    if (map->has_flag(tile_pt, TileFlags::SOLID)) {
      Rect rb(tile_pt.x * 8, tile_pt.y * 8, 8, 8);
      rb.x -= pt.x;
      rb.y -= pt.y;
...
void load_assets() {
  ...
  map = new TiledMap(asset_map, screen.sprites);

  auto environment_layer_index = map->get_layer_index("environment");
  map->add_flags(environment_layer_index, { 8, 59, 31, 47, 28, 29, 44, 45, 60, 48, 49, 50, 64, 66, 80, 81, 82, 56, 57, 58, 72, 74, 88, 89, 90, 61, 62, 63, 77, 79, 93, 94, 95 }, TileFlags::SOLID);
  map->add_flags(environment_layer_index, 7, TileFlags::LADDER);
  map->add_flags(environment_layer_index, { 16, 55, 223 }, TileFlags::WATER);
}
...
void draw_layer(TileLayer *layer) {
  layer->transform = Mat3::translation(Vec2(screen_to_world(Point(0, 0))));
  layer->draw(&screen, {{0, 0}, screen.bounds});
}
